\subsubsection{Analisis Kebutuhan Audit Trail}

Tahap ini bertujuan untuk mengidentifikasi aktivitas-aktivitas pada sistem DecMed yang perlu dicatat sebagai bagian dari mekanisme \textit{audit trail}. Identifikasi tersebut dilakukan dengan memanfaatkan hasil analisis ancaman menggunakan metode STRIDE yang telah dijelaskan pada bagian sebelumnya. Setiap ancaman yang teridentifikasi dianalisis untuk menentukan bukti digital (\textit{digital evidence}) yang diperlukan dalam mendukung proses audit maupun investigasi insiden keamanan. Berdasarkan kebutuhan bukti tersebut, kemudian ditentukan aktivitas sistem yang perlu direkam sebagai \textit{audit event}. Hasil dari tahap ini berupa daftar \textit{audit event} beserta atribut-atribut audit yang diperlukan sebagai dasar dalam perancangan mekanisme pengumpulan dan penyimpanan \textit{audit trail} pada sistem DecMed. Hasil pemetaan antara ancaman, kebutuhan bukti digital, dan audit event yang dihasilkan ditunjukkan pada Tabel \ref{tab:threat-event-mapping}.
...
\paragraph{Identifikasi Atribut Audit}
...
Field \texttt{record\_id} dan \texttt{prev\_record\_hash} tidak secara eksplisit disyaratkan oleh kontrol AU-3. Kedua atribut tersebut ditambahkan untuk memenuhi kebutuhan rancangan \textit{audit trail} pada penelitian ini, khususnya dalam menjamin identitas unik setiap \textit{audit record} serta menjaga integritas rangkaian \textit{audit record} melalui mekanisme \textit{hash chaining}. Penambahan atribut tersebut juga ditujukan untuk mengatasi permasalahan pada \textit{PatientAccessLog} yang telah diidentifikasi pada analisis masalah, yaitu belum tersedianya jaminan bahwa riwayat aktivitas bersifat \textit{append-only} dan \textit{immutable}.

\begin{xltabular}
{\textwidth}
{|>{\centering\arraybackslash}p{1.2cm}|X|>{\raggedright\arraybackslash}p{5.2cm}|}
