% =============================================================
% File: chapters/ta/chapter-3/analisis-solusi.tex
%
% PERUBAHAN:
%   - Pembuka: "sistem audit log terintegrasi" → "mekanisme audit log yang terintegrasi"
%   - "rekam jejak (audit log) yang ideal" → "audit log yang ideal"
%   - Caption tabel penyimpanan: "Audit Log" → "Audit Log"
%   - "Layanan audit log tersentralisasi" → "Layanan audit log tersentralisasi"
%   - S-TA-03: "immutability log" → "tamper-evident dan immutability log"
%   - Paragraf satu baris
% =============================================================
...
\hline
\textbf{\textit{Source}} & \textbf{Jumlah \textit{Event}} & \textbf{\textit{Event} yang Berasal dari \textit{Source}} \\
\hline
\endfirsthead
\hline
\textbf{\textit{Source}} & \textbf{Jumlah \textit{Event}} & \textbf{\textit{Event} yang Berasal dari \textit{Source}} \\
...
PRE Server & 7 & EV7, EV8, EV9, EV10, EV11, EV12, EV13 \\
\hline
Patient Client & 5 & EV1, EV2, EV3, EV8, EV9 \\
\hline
Hospital Client & 4 & EV1, EV4, EV5, EV8, EV9 \\
\hline
Ministry Client & 3 & EV6, EV8, EV9 \\
...
Dalam menentukan mekanisme pengiriman, dianalisis dua dimensi teknis pengiriman data. Pertama, dari segi pola komunikasi, dipertimbangkan antara pendekatan \textit{Pull-based} (\textit{polling}) dan \textit{Push-based}. Pendekatan \textit{Pull-based} memberikan kendali beban bagi pengumpul, tetapi menimbulkan latensi pencatatan dan memaksa \textit{event source} menyediakan penyimpanan sementara lokal. Sebaliknya, pendekatan \textit{Push-based} memungkinkan log dikirimkan secara langsung sesaat setelah kejadian berlangsung. Kedua, dari segi sifat eksekusi, dipertimbangkan antara \textit{Synchronous} dan \textit{Asynchronous}. Pengiriman \textit{Synchronous} memastikan log diterima sebelum transaksi utama selesai, tetapi menambah latensi dan berisiko menggagalkan transaksi utama jika pengumpul bermasalah. Sementara itu, pengiriman \textit{Asynchronous} bersifat \textit{non-blocking} sehingga tidak mengganggu \textit{response time} transaksi klinis maupun kriptografis utama.

\subsubsection{Analisis Keaslian dan Pencegahan \textit{Event} Palsu}
\label{subsubsec:analisis-non-repudiation}

Penggunaan protokol komunikasi terenkripsi seperti TLS/HTTPS hanya
mengamankan jalur pengiriman (\textit{transport layer}), tetapi belum
memberikan jaminan bahwa \textit{event} yang diterima ALS benar-benar
berasal dari komponen sistem yang sah. Tanpa mekanisme verifikasi di
tingkat aplikasi, ALS tidak dapat membedakan \textit{event} yang
dikirimkan oleh komponen DecMed yang legitim dengan \textit{event}
palsu yang disisipkan oleh pihak tidak berwenang. Untuk mengatasi
keterbatasan tersebut, setiap \textit{event source} diwajibkan
menandatangani \textit{payload event} sebelum dikirim ke ALS, sehingga
ALS dapat memverifikasi keaslian setiap \textit{event} yang masuk
sebelum dicatat ke dalam audit log.

Mekanisme penandatanganan yang dipilih adalah tanda tangan digital
berbasis kunci privat IOTA (\texttt{IotaKeyPair}) milik aktor yang
terlibat pada saat kejadian berlangsung. Pendekatan ini memiliki
keunggulan dibandingkan alternatif seperti penggunaan kunci Ed25519
yang dibangkitkan secara acak pada saat inisialisasi komponen: kunci
privat IOTA sudah terikat pada identitas aktor yang terdaftar pada
\textit{ledger} IOTA, sehingga tanda tangan yang dihasilkan dapat
diverifikasi secara kriptografis terhadap identitas yang terregistrasi
pada sistem. Dengan demikian, ALS dapat memastikan bahwa setiap
\textit{event} yang masuk ke dalam audit log berasal dari komponen
atau aktor yang memang terdaftar dalam sistem DecMed, bukan dari
sumber yang tidak dikenal atau tidak berwenang.

Dengan menerapkan mekanisme \textit{signed payload} berbasis kunci
IOTA aktor, ALS dapat menolak seluruh \textit{event} yang tidak
memiliki tanda tangan valid sebelum diproses lebih lanjut. Hal ini
menjamin bahwa isi audit log hanya memuat catatan aktivitas yang
berasal dari sumber yang terverifikasi, sehingga keaslian dan
keandalan audit log sebagai catatan aktivitas sistem dapat terjaga.

\subsubsection{Analisis Kerahasiaan Log dalam Transit}
\label{subsubsec:analisis-kerahasiaan-log-transit}

Selain integritas dan \textit{non-repudiation}, \textit{audit event} yang dikirimkan dari \textit{event source} ke ALS berpotensi mengandung informasi sensitif, seperti identitas aktor, identitas objek yang diakses, maupun detail operasi yang dilakukan. Apabila \textit{event} dikirimkan dalam bentuk \textit{plaintext} atau hanya mengandalkan enkripsi pada lapisan transport (TLS), maka ALS sebagai pihak penerima akan memiliki akses penuh terhadap isi \textit{event} pada saat penerimaan. Pemisahan antara kemampuan menerima \textit{event} dan kemampuan membaca isi \textit{event} menjadi kebutuhan yang perlu dipertimbangkan, terutama untuk memastikan bahwa log yang tersimpan tidak dapat dibaca oleh pihak yang seharusnya hanya berperan sebagai pengumpul dan penyimpan, bukan pembaca.

Untuk memenuhi kebutuhan ini, diterapkan mekanisme enkripsi \textit{end-to-end} pada level aplikasi menggunakan skema \textit{hybrid encryption}. Setiap \textit{payload} \textit{audit event} dienkripsi menggunakan AES-256-GCM dengan kunci simetris yang dibangkitkan secara acak untuk setiap pengiriman. Kunci AES tersebut kemudian dienkripsi menggunakan kunci publik P-256 milik ALS melalui mekanisme ECIES (\textit{Elliptic Curve Integrated Encryption Scheme}) yang menggabungkan pertukaran kunci Diffie-Hellman efemermal dan fungsi derivasi kunci berbasis SHA-256. Hanya ALS yang memiliki kunci privat P-256 yang dapat mendekripsi kunci AES dan selanjutnya mendekripsi isi \textit{event}. Urutan operasi yang diterapkan adalah \textit{Encrypt-then-Sign}: \textit{payload} dienkripsi terlebih dahulu, kemudian \textit{ciphertext} yang dihasilkan ditandatangani menggunakan kunci privat IOTA aktor. Urutan ini dipilih karena tanda tangan atas \textit{ciphertext} membuktikan bahwa pengirim adalah pihak yang membuat \textit{ciphertext} tersebut, bukan sekadar meneruskan ciphertext milik pihak lain.

Konsekuensi dari penerapan enkripsi \textit{end-to-end} ini adalah bahwa berkas log yang tersimpan secara lokal di ALS maupun yang diarsipkan ke IPFS memuat \textit{audit event} dalam bentuk terenkripsi. Dekripsi hanya dilakukan pada saat pembacaan log untuk keperluan audit oleh pihak yang berwenang dengan menggunakan kunci privat ALS, bukan pada saat penerimaan \textit{event}. Pendekatan ini memisahkan tanggung jawab antara pengumpulan bukti (\textit{collection}) dan pembacaan bukti (\textit{inspection}), serta memberikan jaminan kerahasiaan isi \textit{audit event} selama masa penyimpanan.
...
\begin{enumerate}
    \item Berkas Audit Log Utama (\textit{Off-chain} IPFS): Seluruh \textit{payload} dan berkas \textit{audit record} terstruktur akan disimpan ke dalam IPFS. Pendekatan ini memanfaatkan infrastruktur IPFS yang sudah ada pada DecMed untuk menangani penyimpanan berkas log berukuran besar secara efisien.
    \item Metadata Verifikasi (\textit{On-chain} Move): \textit{Content Identifier} (CID) dari IPFS beserta metadata penting (seperti \textit{timestamp}, ID transaksi, dan \textit{digest} integritas) disimpan ke dalam \textit{smart contract} Move pada jaringan IOTA.
\end{enumerate}

Pada \textit{smart contract} Move yang baru, tidak akan ada fungsi modifikasi/pembaruan log dan struktur objek dirancang sebagai \textit{immutable object}. Dengan demikian, jika ada upaya mengubah isi berkas log di IPFS, \textit{hash} berkas tersebut tidak akan cocok lagi dengan metadata yang terkunci di \textit{smart contract} Move, sehingga jaminan integritas dan \textit{tamper-evident} bukti digital (M-TA-03) dapat ditegakkan.
...
Dalam skema \textit{hash chaining}, setiap kali sebuah entri \textit{audit
record} baru dibentuk, struktur data wajib menyertakan nilai \textit{hash} dari
entri \textit{audit record} sebelumnya pada bidang \texttt{prev\_record\_hash}.
Dengan pola keterikatan ini, setiap entri log secara kriptografis mengunci entri
sebelumnya. Apabila penyerang mengubah, menyisipkan, atau menghapus entri yang
masih diikuti oleh entri lain, konsistensi rantai \textit{hash} pada entri
berikutnya akan rusak.
 
Melalui penerapan \textit{hash chaining} ini, berkas log sementara pada
lingkungan lokal menjadi bersifat \textit{tamper-evident} terhadap perubahan di
dalam rantai. Akan tetapi, \textit{hash chaining} tidak dapat mendeteksi
penghapusan entri terakhir, karena rantai yang terpotong di bagian ujung tetap
konsisten secara internal. Kelemahan ini memerlukan \textit{anchor} eksternal,
yaitu pencatatan nilai \textit{hash} ujung rantai pada media yang tidak dapat
diubah oleh pengelola log, yang pada Tugas Akhir ini dipenuhi melalui publikasi
metadata rotasi ke IOTA.
...
S-TA-03 & Penjaminan sifat \textit{tamper-evident} dan \textit{immutability} log bertingkat melalui \textit{hash chaining} pada berkas lokal dan penguncian metadata di \textit{smart contract} Move. & M-TA-03 \\ \hline
