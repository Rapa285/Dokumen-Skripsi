...
Namun, pengiriman asinkron semata belum cukup untuk menjamin bahwa setiap \textit{event} yang terjadi benar-benar akan sampai ke ATS. Apabila ATS sedang tidak tersedia saat \textit{event} dibangkitkan, pengiriman langsung akan gagal dan \textit{event} tersebut berpotensi hilang secara permanen. Untuk mengatasi kerentanan ini, setiap \textit{event source} dilengkapi dengan mekanisme \textit{persistent queue} berbasis berkas lokal. Setiap \textit{event} yang akan dikirim terlebih dahulu ditulis ke antrian persisten di disk sebelum pengiriman ke ATS dicoba. Apabila pengiriman langsung berhasil, entri tersebut dihapus dari antrian. Apabila gagal, entri tetap berada di antrian dan sebuah \textit{background worker} yang berjalan secara berkala akan mencoba mengirim ulang seluruh entri yang tersisa menggunakan strategi \textit{exponential backoff} hingga berhasil atau mencapai batas percobaan maksimum. Kombinasi pengiriman asinkron dengan antrian persisten ini menjamin bahwa setiap \textit{audit event} yang telah dibangkitkan akan terkirim ke ATS, bahkan dalam kondisi ATS mengalami gangguan sementara.

\subsubsection{Analisis Jaminan Keaslian dan \textit{Non-Repudiation} Log}
\label{subsubsec:analisis-jaminan-keasilan-log}
Mengingat bahwa beberapa \textit{event} dapat bersumber dari beberapa \textit{event source}, maka diperlukan sebuah mekanisme untuk secara jelas menyatakan sumber dari \textit{audit event} dan menjamin bahwa sumber tidak dapat menyangkal pengiriman \textit{event} tersebut.
