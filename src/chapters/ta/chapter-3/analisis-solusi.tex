...
%
% PERUBAHAN:
%   - Pembuka: "sistem audit trail terintegrasi" → "mekanisme audit log yang terintegrasi"
%   - "rekam jejak (audit trail) yang ideal" → "audit log yang ideal"
%   - Caption tabel penyimpanan: "Audit Trail" → "Audit Log"
%   - "Layanan audit trail tersentralisasi" → "Layanan audit log tersentralisasi"
...
\subsubsection{Analisis Keaslian dan \textit{Non-Repudiation} Log}
\label{subsubsec:analisis-non-repudiation}

Penggunaan protokol komunikasi terenkripsi seperti TLS/HTTPS hanya mengamankan jalur pengiriman (\textit{transport layer}), tetapi belum memberikan jaminan integritas dan pembuktian asal pesan secara independen di tingkat aplikasi. Untuk mengatasi keterbatasan tersebut, setiap \textit{event source} diwajibkan menandatangani \textit{payload} \textit{event} sebelum dikirim ke ATS.

Mekanisme penandatanganan yang dipilih adalah tanda tangan digital berbasis kunci privat IOTA (\texttt{IotaKeyPair}) milik aktor yang terlibat pada saat kejadian berlangsung. Pendekatan ini memiliki keunggulan krusial dibandingkan alternatif seperti penggunaan kunci Ed25519 yang dibangkitkan secara acak pada saat inisialisasi komponen: kunci privat IOTA sudah terikat pada identitas aktor yang terdaftar pada \textit{ledger} IOTA, sehingga tanda tangan yang dihasilkan dapat diverifikasi secara kriptografis terhadap identitas tersebut. Dengan demikian, apabila sebuah \textit{event} telah ditandatangani menggunakan kunci privat IOTA seorang aktor, aktor tersebut tidak dapat menyangkal bahwa \textit{event} tersebut berasal dari dirinya, karena kepemilikan kunci privat tersebut secara langsung mencerminkan identitas yang teregistrasi pada sistem. Penguatan \textit{non-repudiation} ini merupakan perbaikan signifikan terhadap mekanisme sebelumnya di mana kunci penandatanganan dibangkitkan secara acak dan tidak terikat pada identitas aktor mana pun.

Dengan menerapkan mekanisme \textit{signed payload} berbasis kunci IOTA aktor, komponen pengumpul dapat memverifikasi otentisitas pengirim dan integritas isi log sebelum diproses lebih lanjut, sehingga mencegah penyusupan data palsu dan menjamin sifat \textit{non-repudiation} yang lebih kuat.

\subsubsection{Analisis Kerahasiaan Log dalam Transit}
\label{subsubsec:analisis-kerahasiaan-log-transit}

Selain integritas dan \textit{non-repudiation}, \textit{audit event} yang dikirimkan dari \textit{event source} ke ATS berpotensi mengandung informasi sensitif, seperti identitas aktor, identitas objek yang diakses, maupun detail operasi yang dilakukan. Apabila \textit{event} dikirimkan dalam bentuk \textit{plaintext} atau hanya mengandalkan enkripsi pada lapisan transport (TLS), maka ATS sebagai pihak penerima akan memiliki akses penuh terhadap isi \textit{event} pada saat penerimaan. Pemisahan antara kemampuan menerima \textit{event} dan kemampuan membaca isi \textit{event} menjadi kebutuhan yang perlu dipertimbangkan, terutama untuk memastikan bahwa log yang tersimpan tidak dapat dibaca oleh pihak yang seharusnya hanya berperan sebagai pengumpul dan penyimpan, bukan pembaca.

Untuk memenuhi kebutuhan ini, diterapkan mekanisme enkripsi \textit{end-to-end} pada level aplikasi menggunakan skema \textit{hybrid encryption}. Setiap \textit{payload} \textit{audit event} dienkripsi menggunakan AES-256-GCM dengan kunci simetris yang dibangkitkan secara acak untuk setiap pengiriman. Kunci AES tersebut kemudian dienkripsi menggunakan kunci publik P-256 milik ATS melalui mekanisme ECIES (\textit{Elliptic Curve Integrated Encryption Scheme}) yang menggabungkan pertukaran kunci Diffie-Hellman efemermal dan fungsi derivasi kunci berbasis SHA-256. Hanya ATS yang memiliki kunci privat P-256 yang dapat mendekripsi kunci AES dan selanjutnya mendekripsi isi \textit{event}. Urutan operasi yang diterapkan adalah \textit{Encrypt-then-Sign}: \textit{payload} dienkripsi terlebih dahulu, kemudian \textit{ciphertext} yang dihasilkan ditandatangani menggunakan kunci privat IOTA aktor. Urutan ini dipilih karena tanda tangan atas \textit{ciphertext} membuktikan bahwa pengirim adalah pihak yang membuat \textit{ciphertext} tersebut, bukan sekadar meneruskan ciphertext milik pihak lain.

Konsekuensi dari penerapan enkripsi \textit{end-to-end} ini adalah bahwa berkas log yang tersimpan secara lokal di ATS maupun yang diarsipkan ke IPFS memuat \textit{audit event} dalam bentuk terenkripsi. Dekripsi hanya dilakukan pada saat pembacaan log untuk keperluan audit oleh pihak yang berwenang dengan menggunakan kunci privat ATS, bukan pada saat penerimaan \textit{event}. Pendekatan ini memisahkan tanggung jawab antara pengumpulan bukti (\textit{collection}) dan pembacaan bukti (\textit{inspection}), serta memberikan jaminan kerahasiaan isi \textit{audit event} selama masa penyimpanan.
