...
\texttt{AuditRecord} merupakan satuan penyimpanan pada berkas log. Setiap
\textit{record} memuat atribut rantai yang dibentuk oleh ALS, yaitu
\texttt{record\_id}, \texttt{timestamp}, \texttt{prev\_record\_hash}, dan
\texttt{record\_hash}, serta \texttt{EncryptedSignedEvent} yang diterima dari
\textit{event source} tanpa perubahan. Dengan demikian, berkas log menyimpan
atribut rantai dalam bentuk \textit{plaintext}, sedangkan isi \textit{event}
tetap tersimpan sebagai \textit{ciphertext} beserta tanda tangan dan alamat IOTA
penanda tangannya. Implementasi \texttt{AuditRecord} ditunjukkan pada
Kode~\ref{code:audit-record}.
 
\begin{listing}[ht]
\caption{Implementasi \texttt{AuditRecord}}
...
Karena \texttt{record\_hash} dihitung atas \texttt{EncryptedSignedEvent},
mekanisme \textit{hash-chaining} tetap menjamin sifat \textit{tamper-evident}
tanpa ALS perlu membuka isi \textit{event}.
...
Untuk mengurangi risiko kehilangan \textit{event} akibat kegagalan jaringan
...
Fase pertama adalah verifikasi tanda tangan melalui
\texttt{Utils::verify\_event\_signature}: ALS memverifikasi
\texttt{IotaSignature} pada bidang \texttt{signature} terhadap \textit{byte}
\textit{ciphertext}, menggunakan \texttt{iota\_address} yang disertakan sebagai
identitas penanda tangan. Apabila verifikasi gagal, \textit{event} ditolak dan
tidak diteruskan ke tahap berikutnya.
 
Fase kedua adalah penerusan ke antrian internal: \texttt{EncryptedSignedEvent}
yang telah terverifikasi diteruskan \emph{tanpa didekripsi} ke antrian asinkron
\texttt{tokio::sync::mpsc} berkapasitas 10.000 \textit{event} melalui
\texttt{state.audit\_tx.send(event).await}. ALS tidak membuka isi \textit{event}
pada saat penerimaan, sehingga atribut sensitif seperti \texttt{actor\_id},
\texttt{target\_object}, dan \texttt{details} tetap berada dalam bentuk
\textit{ciphertext} sejak dikirim oleh \textit{event source} hingga tersimpan
pada berkas log. Pemisahan antrian ini juga memungkinkan ALS menerima
\textit{event} berikutnya tanpa menunggu penulisan ke berkas log selesai.

Karena \textit{handler} mengembalikan respons berhasil setelah
\texttt{audit\_tx.send} selesai, respons tersebut menandakan bahwa
\textit{event} telah diterima ke antrian internal, bukan bahwa \textit{event}
telah ditulis ke berkas log. \textit{Event} yang masih berada di antrian
internal pada saat ALS berhenti secara tidak terduga tidak akan tercatat,
sementara salinannya pada \texttt{AlsQueue} di sisi pengirim telah dihapus.
...
\hline
Penerusan \texttt{EncryptedSignedEvent} tanpa dekripsi ke antrian asinkron internal (kapasitas 10.000) & Terimplementasi. \\
\hline
\end{tabular}
\end{table}
...
Nilai \texttt{prev\_record\_hash} disimpan sebagai bidang pada
\texttt{AuditLogger} di memori dan diinisialisasi dengan \texttt{None} pada saat
aplikasi dimulai. Implementasi tidak memulihkan nilai tersebut dari
\textit{record} terakhir pada berkas log aktif maupun dari
\texttt{final\_record\_hash} pada IOTA setelah ALS dimulai ulang. Akibatnya, setiap \textit{restart} memulai rantai baru:
\textit{record} pertama setelah \textit{restart} memiliki
\texttt{prev\_record\_hash} bernilai \texttt{null} meskipun berkas log aktif
masih berisi \textit{record} sebelumnya.
...
\textit{Patient Client}, \textit{Hospital Client}, dan \textit{Ministry Client} merupakan aplikasi \textit{client} berbasis Tauri. Pada ketiga aplikasi tersebut, modul \texttt{als} diimplementasikan pada sisi \textit{backend} Rust dari aplikasi Tauri, sehingga enkripsi, penandatanganan, dan pengiriman \textit{event} tidak terekspos ke lapisan antarmuka SvelteKit/TypeScript. Distribusi \textit{event} berdasarkan \textit{event source} ditunjukkan pada Tabel~\ref{tab:event-source-distribution}.
