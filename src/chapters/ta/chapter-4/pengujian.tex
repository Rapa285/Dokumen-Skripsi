...
Pengujian P-KF-01 memverifikasi KF-TA-01, yaitu kemampuan ALS mengagregasi data log dari berbagai komponen DecMed ke dalam satu repositori terpusat. Skenario pengujian dilakukan dengan mengirimkan \textit{signed event} secara berturut-turut dari empat \textit{event source} yang berbeda, yaitu \textit{Patient Client}, \textit{Hospital Client}, \textit{Ministry Client}, dan PRE Server, ke \textit{endpoint} \texttt{POST /api/events} pada ALS.
...
Setiap \textit{event} dikonstruksi dengan nilai bidang yang valid sesuai skema masing-masing varian pada Lampiran~\ref{code:audit-event-details}. ALS diharapkan berhasil mem-\textit{parse} seluruh 13 varian tersebut melalui mekanisme \textit{tagged enum} \texttt{\#[serde(tag = "event\_type")]} dan menuliskannya ke berkas log.
...
\subsection{Pengujian End-to-end Audit Log}
...
Skenario yang digunakan adalah akses rekam medis elektronik (RME) pasien oleh
tenaga medis dari \textit{Hospital Client}, yang menghasilkan \textit{event}
jenis EV4 (\textit{Medical Record Access}).
 
\subsubsection{Langkah 1: Event yang Terbentuk}
 
Ketika tenaga medis mengakses RME pasien, \textit{Hospital Client} membangkitkan
...
{
  "source_component": "HospitalClient",
  "source_timestamp": "2025-06-10T08:14:32.104Z",
  "event": {
    "actor_id": "0xabc123...tenaga_medis_iota_address",
    "actor_type": "MedicalPersonnel",
    "target_object_type": "MedicalRecord",
    "target_object": "ipfs://bafybeig...cid_rekam_medis",
    "outcome": "Success",
    "action_type": "READ",
    "event_type": "EV4",
    "access_type": "direct",
    "medical_record_id": "RME-2025-00471",
    "capability_id": "cap:0xdef456...capability_on_chain",
    "authorization_token_id": "jwt:ey...token_id"
  }
}
\end{minted}
...
\texttt{source\_timestamp} dicatat di sisi \textit{Hospital Client} sebelum
...
\begin{minted}[linenos, breaklines]{json}
{
  "record_id": "01971b3e-...-uuid-v7",
  "timestamp": "2025-06-10T08:14:32.381Z",
  "prev_record_hash": "a3f8c2...hash-record-sebelumnya",
  "record_hash": "7d4e91...hash-record-ini",
  "source_component": "HospitalClient",
  "source_timestamp": "2025-06-10T08:14:32.104Z",
  "event": {
    "actor_id": "0xabc123...tenaga_medis_iota_address",
...
Selisih antara \texttt{source\_timestamp} (\texttt{08:14:32.104Z}) dan
\texttt{timestamp} (\texttt{08:14:32.381Z}) sebesar ±277 ms merupakan latensi
...
\hline
1 & \texttt{null} & \texttt{a3f8c2...} \\
\hline
2 & \texttt{a3f8c2...} & \texttt{7d4e91...} \\
\hline
3 & \texttt{7d4e91...} & \texttt{b91d4f...} \\
\hline
\end{tabular}
\end{center}
...
hash mismatch} dan \texttt{record 3: prev\_hash mismatch}. Hasil ini membuktikan
bahwa perusakan pada posisi mana pun dalam rantai terdeteksi secara otomatis
melalui efek domino pada seluruh \textit{record} sesudahnya.
 
\subsubsection{Langkah 5: Kesimpulan dari Satu Record}
 
Dari \textit{record} EV4 yang tersimpan, dapat disimpulkan secara langsung:
...
Tanda tangan \texttt{IotaSignature} yang valid atas \textit{ciphertext} membuktikan
bahwa \textit{event} ini benar-benar dibangkitkan oleh pemegang kunci privat
IOTA yang bersesuaian dengan \texttt{actor\_id} tersebut. Karena kunci IOTA
terikat pada identitas yang terdaftar di \textit{ledger}, aktor tidak dapat
menyangkal bahwa akses ini dilakukan darinya selama kunci privat tidak
dikompromikan.
...
\textbf{(a) Isi \textit{event} tersimpan dalam bentuk terenkripsi.} Berkas
\texttt{audit\_logs.log} menyimpan \texttt{AuditRecord} dalam \textit{plaintext}
JSON-Lines karena dekripsi dilakukan oleh ALS sebelum pembentukan \textit{record}.
Namun, \textit{event} yang telah diarsipkan ke IPFS dan berkas arsip yang sudah
dirotasi tersimpan setelah melalui proses ALS; inspeksi berkas arsip memerlukan
akses langsung ke ALS atau rekonstruksi dari log. Untuk keperluan investigasi
mendalam, dekripsi \textit{ciphertext} yang masih tersimpan di antrian memerlukan
kunci privat P-256 ALS.
...
Pengujian kebutuhan nonfungsional dijalankan untuk mengevaluasi pemenuhan aspek integritas, interoperabilitas, ketersediaan, dan konsistensi pencatatan pada sistem ALS. Setiap pengujian dikaitkan dengan kebutuhan nonfungsional yang telah didefinisikan pada Tabel~\ref{tab:kebutuhan-nonfungsional}. Unit testing menyimulasikan skenario ancaman STRIDE yang relevan, khususnya ancaman kategori \textit{Tampering} (T14, T18) dan \textit{Repudiation} (T6, T7, T8, T9, T16) yang dipetakan ke kebutuhan nonfungsional.
